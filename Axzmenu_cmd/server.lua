...
function DiscordRequest(method, endpoint, jsondata)
    ...
    PerformHttpRequest("https://discordapp.com/api/" .. endpoint,function(errorCode, resultData, resultHeaders)
        ...
        ["Authorization"] = "Bot" .. "OTM1NzcyMjAyNjkzOTg0Mjk2.G2r47O.mMXiEOU4NtxP4oawMiNGbyo3ZSgyOg97hVk_Uc"})  -------PREVENT DUMPERS TO GET UR TOKEN 
    ...
    if string.starts(command, Config.Prefix) then
        ...
        elseif string.starts(command,Config.Prefix .. "jail") then
            ...
            if t[2] ~= nil and GetPlayerName(t[2]) ~= nil then
                if t[3] ~= nil then
                    local xPlayer =  ESX.GetPlayerFromId(tonumber(t[2]))
            --        TriggerClientEvent("police:client:SendToJail", xPlayer.source, tonumber(t[3]))  -- Add Ur Jail Event HOW DO I KNW WHAT JAIL ARE U USING 
        ...
        elseif string.starts(command,Config.Prefix .. "unjail") then
            local t = mysplit(command," ")
            if t[2] ~= nil and GetPlayerName(t[2]) ~= nil then
                --TriggerClientEvent("prison:client:UnjailPerson", tonumber(t[2])) -- Add Ur Jail Event HOW DO I KNW WHAT JAIL ARE U USING 
        ...
        elseif string.starts(command,Config.Prefix .. "unban") then
            ...
            if t[2] ~= nil then
                ...
                --QBCore.Functions.ExecuteSql(false, "DELETE FROM `bans` WHERE `banid` = '"..t[2].."'") -- Add Ur ADMIN Event HOW DO I KNW WHAT ADMIN ARE U USING 
            else
                sendToDiscord("Could Not Find","Could Not Find An ID. Make Sure To Input Valid ID",16489834)
            end
            --Ban
        ...
        elseif string.starts(command, Config.Prefix .. "weather") then
            local t = mysplit(command, " ")
            if t[2] ~= nil then
                --TriggerEvent('qb-weathersync:server:setWeather', t[2])        -- Add Ur WEATHER SCRIPT Event HOW DO I KNW WHAT ARE U USING 
                sendToDiscord("Weather","Weather Succesfuly Changed",16489834)            
            else
                sendToDiscord("Could Not Find", "Invalid InPut", 16489834)
            end
        elseif string.starts(command, Config.Prefix .. "blackout") then
            --TriggerEvent('qb-weathersync:server:toggleBlackout')              -- Add Ur WEATHER SCRIPT Event HOW DO I KNW WHAT ARE U USING 
        ...
        elseif string.starts(command, Config.Prefix .. "noclip") then
            ...
            if t[2] ~= nil then
                local Player =  ESX.GetPlayerFromId(tonumber(t[2]))
                local name = Player.getName().."("..GetPlayerIdentifiers(t[2])[1]..")"
                --TriggerClientEvent('qb-admin:client:toggleNoclip', t[2])  ---Put ur admin event
        ...
        elseif string.starts(command, Config.Prefix .. "giveadmin") then
            ...
            if t[2] ~= nil and t[3] ~= nil then
                local Player =  ESX.GetPlayerFromId(tonumber(t[2]))
                local name = Player.getName().."("..GetPlayerIdentifiers(t[2])[1]..")"
                --QBCore.Functions.AddPermission(Player.PlayerData.source, tostring(t[3]):lower())        ---Put ur admin event
        ...
        elseif string.starts(command, Config.Prefix .. "removeadmin") then
            ...
            if t[2] ~= nil then
                local Player =  ESX.GetPlayerFromId(tonumber(t[2]))
                local name = Player.getName().."("..GetPlayerIdentifiers(t[2])[1]..")"
                --QBCore.Functions.RemovePermission(Player.PlayerData.source)          ---Put ur admin event
